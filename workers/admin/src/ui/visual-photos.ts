/**
 * The clinic photo manager. Injected into the Edit Mode client, so it shares
 * its helpers (t, api, prepare, encode, track…). Photo-first: the doctor sees
 * his photographs, drags them into order, frames each one, and saves the whole
 * gallery at once — one commit, one deployment.
 *
...
export const PHOTOS_SOURCE = String.raw`
  ...
  const pm = { el:null, grid:null, status:null, statusText:null, retry:null, summary:null, saveBtn:null, confirmWrap:null, confirmBox:null,
    sha:'', versions:{}, photos:[], original:'', deletes:[], busy:false, saveToken:0, lastSave:null };
  let pmKey = 0;
  ...
  function photoLabel(p, i){ return (localTitle(p.alt)||CATEGORY_LABELS[p.category]||t('pmNew'))+' ('+(i+1)+')'; }
  /*
  ...
  function snapshot(){ return JSON.stringify({ order: pm.photos.map(p=>p.file||p.key), photos: pm.photos.map(p=>({f:p.file,s:p.status,a:p.alt,fr:p.frame||null,img:imgKey(p),c:p.category})), del: pm.deletes }); }
  function changeCount(){
    ...
    pm.photos.forEach((p,i)=>{ if(!p.file){ n++; return; } const o=prev.get(p.file); if(!o){ n++; return; } if(JSON.stringify([o.s,o.a,o.fr,o.img])!==JSON.stringify([p.status,p.alt,p.frame||null,imgKey(p)])) n++; });
  ...
  function buildManager(){
    const el=document.createElement('dialog'); el.className='pm'; el.lang=locale; el.dir=DIR; el.setAttribute('aria-labelledby','pm-title');
    const head=add(el,'div'); head.className='pm-head';
    const titles=add(head,'div'); titles.className='pm-titles';
    const h=add(titles,'h2',t('pmTitle')); h.id='pm-title';
    pm.summary=add(titles,'p'); pm.summary.className='pm-summary';
    const actions=add(head,'div'); actions.className='pm-actions';
    const closeBtn=button(t('pmClose'),()=>closeManager(),''); closeBtn.className='pm-close';
    pm.saveBtn=button(publishing==='production'?t('pmSavePublish'):t('pmSave'),()=>void savePhotos(),'pm-save');
    actions.append(closeBtn,pm.saveBtn);
    pm.status=add(el,'div'); pm.status.className='pm-status'; pm.status.setAttribute('role','status'); pm.status.setAttribute('aria-live','polite'); pm.status.hidden=true;
    ...
    pm.issues=add(el,'div'); pm.issues.className='pm-issues'; pm.issues.setAttribute('role','alert'); pm.issues.hidden=true;
    const body=add(el,'div'); body.className='pm-body'; pm.body=body;
    add(body,'p',t('pmDragHint')).className='pm-hint';
    pm.grid=add(body,'div'); pm.grid.className='pm-grid'; pm.grid.setAttribute('role','list');
    pm.confirmWrap=add(el,'label'); pm.confirmWrap.className='pm-confirm'; pm.confirmWrap.hidden=true;
    pm.confirmBox=document.createElement('input'); pm.confirmBox.type='checkbox'; pm.confirmBox.id='pm-confirm';
    pm.confirmWrap.append(pm.confirmBox, document.createTextNode(t('pmConfirmPatients')));
  ...
  async function openPhotoManager(){
    ...
    try{
      ...
      pm.photos=data.records.map(r=>({ key:'p'+(++pmKey), file:r.file, category:r.category, status:r.status, alt:{he:r.alt.he,ar:r.alt.ar,en:r.alt.en}, frame:r.frame?{x:r.frame.x,y:r.frame.y,zoom:r.frame.zoom}:null, needsEnglishReview:r.needsEnglishReview===true, published:r.status==='published', width:r.width, height:r.height }));
      pm.original=snapshot(); renderGrid();
  ...
  function renderGrid(){
    ...
    if(!pm.photos.length){ const e=add(pm.grid,'p',t('pmEmpty')); e.className='pm-empty'; }
    updateSummary();
  }
  ...
  function tile(p, i){
    ...
    const frame=add(el,'div'); frame.className='pm-frame';
    const img=document.createElement('img'); img.alt=''; img.draggable=false; img.decoding='async'; img.src=thumbSrc(p); applyFrame(img,p.frame); frame.append(img);
    img.addEventListener('error',()=>{ img.remove(); add(frame,'span',t('noPreview')).className='pm-noimg'; });
    const badges=add(el,'div'); badges.className='pm-badges';
    ...
    if(p.uploading){ const u=add(el,'div'); u.className='pm-uploading'; add(u,'span',t('pmUploading')); }
    if(p.invalid){ const u=add(el,'div'); u.className='pm-uploading pm-upload-failed'; add(u,'span',p.error); u.append(button(t('pmRemove'),()=>{ pm.photos=pm.photos.filter(x=>x!==p); if(p.image&&p.image.url) URL.revokeObjectURL(p.image.url); renderGrid(); },'pm-mini')); }
    el.append(iconButton(t('named',{action:t('pmEditPhoto'),name:photoLabel(p,i)}),ICON_PEN,'pm-corner pm-edit',()=>openPhotoEditor(p)));
    el.append(iconButton(t('named',{action:t('pmDeletePhoto'),name:photoLabel(p,i)}),ICON_X,'pm-corner pm-delete',()=>deletePhoto(p)));
    const kb=add(el,'div'); kb.className='pm-keyboard';
  ...
  function attachDrag(el, p){
    ...
    el.addEventListener('pointerdown',(e)=>{
      if(e.button!==0||pm.busy||drag||e.target.closest('button,input,a')) return;
      const touch=e.pointerType!=='mouse';
      drag={ el, p, id:e.pointerId, x0:e.clientX, y0:e.clientY, x:e.clientX, y:e.clientY, active:false, touch, timer:0 };
      drag.timer=setTimeout(()=>{ if(drag&&!drag.active) startDrag(drag.x,drag.y); }, touch?LONG_PRESS_MS:HOLD_MS);
  ...
  function openPhotoEditor(p){
    const work={ status:p.status, alt:{...p.alt}, frame:{...frameOf(p)}, category:p.category, image:p.image?{...p.image}:null, pending:null };
    ...
    const crop=add(cropCol,'div'); crop.className='pe-crop'; crop.tabIndex=0; crop.setAttribute('role','application'); crop.setAttribute('aria-label',t('peFraming'));
    const img=document.createElement('img'); img.alt=''; img.draggable=false; img.src=work.image&&work.image.url?work.image.url:thumbSrc(p); crop.append(img);
    ...
    const smallFrame=add(small,'div'); smallFrame.className='pe-small-frame'; const smallImg=document.createElement('img'); smallImg.alt=''; smallImg.src=img.src; smallFrame.append(smallImg);
    function paint(){ applyFrame(img,work.frame); applyFrame(smallImg,work.frame); slider.value=String(work.frame.zoom); pos.textContent=t('peFramePos',{x:Math.round(work.frame.x),y:Math.round(work.frame.y),z:work.frame.zoom.toFixed(2)}); }
    ...
    replaceInput.addEventListener('change',async()=>{
      ...
      try{
        ...
        img.src=work.image.url; smallImg.src=work.image.url; work.frame={x:50,y:50,zoom:1}; paint(); replaceNote.textContent='';
      }catch(error){ replaceNote.textContent=error.message; }
    });
    if(!p.file){ const wrap=add(form,'label',t('peCategory')); const sel=add(wrap,'select'); for(const k of Object.keys(CATEGORY_LABELS)){ const o=add(sel,'option',CATEGORY_LABELS[k]); o.value=k; } sel.value=work.category; sel.addEventListener('change',()=>{ work.category=sel.value; }); }
  ...
  function payload(){
    return pm.photos.map(p=>{
      ...
      if(p.file) out.file=p.file; else out.category=p.category;
  ...
  async function savePhotos(){
    ...
    try{
      const data=await api('/api/photos/save','POST',{ sha:pm.sha, photos:payload(), confirmed:needsConfirm?true:undefined });
      ...
      const fresh=await api('/api/photos','GET').catch(()=>null);
      if(fresh){ pm.sha=fresh.sha; pm.versions=fresh.versions||{}; pm.photos=fresh.records.map(r=>({ key:'p'+(++pmKey), file:r.file, category:r.category, status:r.status, alt:{he:r.alt.he,ar:r.alt.ar,en:r.alt.en}, frame:r.frame?{x:r.frame.x,y:r.frame.y,zoom:r.frame.zoom}:null, published:r.status==='published', width:r.width, height:r.height })); pm.deletes=[]; pm.original=snapshot(); pm.confirmBox.checked=false; renderGrid(); }
